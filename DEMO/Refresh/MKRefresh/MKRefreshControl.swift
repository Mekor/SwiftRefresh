...
/// 刷新状态切换的临界点
fileprivate let MKRefreshOffset: CGFloat = 60

class MKRefreshControl: UIControl {

    //刷新控件的父视图，下拉刷新控件应该适用于 tableView 和collectionView
    //父视图addSubView 强引用了self，所以使用weak
    fileprivate weak var scrollView: UIScrollView?
    fileprivate lazy var refrershView: MKRefreshView = MKRefreshView.refreshView()
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if sv.isDragging {
            if height > MKRefreshOffset && refrershView.refreshStatus == .Normal{
                print("放手刷新")
                refrershView.refreshStatus = .Pulling
            }else if height <= MKRefreshOffset && refrershView.refreshStatus == .Pulling{
                print("继续拽")
    ...
    func beginRefreshing(){
        ...
        inset1.top += MKRefreshOffset
        sv.contentInset = inset1
        
        refrershView.parentViewHeight = MKRefreshOffset
    }
